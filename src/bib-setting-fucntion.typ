...
// ---------- 日本語の著者名は姓と名の間に半角スペースを入れて出力 (JPA style) ---------- //
...
#let author-make-arr(biblist, name) = {
  let author_str = biblist.at(name)
  if type(author_str) == array {
      author_str = author_str.sum()
  }
...
#let bib-citet-default(bib_cite_contents, label) = {
  return text(fill: black)[#bib_cite_contents.at(0)] + text(fill: black)[（] + link(label, text(fill: rgb(0, 128, 0))[#bib_cite_contents.at(1)]) + text(fill: black)[）]
}

#let bib-citep-default(bib_cite_contents, label) = {
...
#let bib-citen-default(bib_cite_contents, label) = {
  return link(label, text(fill: rgb(0, 128, 0))[#str(bib_cite_contents.at(2))])
}

#let bib-citefull-default(bib_cite_contents, label) = {
  return link(label, bib_cite_contents.at(3))
}

#let bib-cite-authoronly(bib_cite_contents, label) = {
  return link(label, text(fill: black)[#bib_cite_contents.at(0)])
}

#let bib-cite-yearonly(bib_cite_contents, label) = {
  return link(label, text(fill: rgb(0, 128, 0))[#bib_cite_contents.at(1)])
}

// Raw year only - no parentheses, no suffix letters (a, b, c)
#let bib-cite-yearonly-raw(bib_cite_contents, label) = {
  let year_str = bib_cite_contents.at(1)
  // Convert to string if content
  if type(year_str) == content {
    year_str = if year_str.has("text") { year_str.text } else { repr(year_str) }
  }
  // Remove suffix letters (a, b, c, etc.)
  let raw_year = year_str.replace(regex("[a-z]$"), "")
  return link(label, text(fill: rgb(0, 128, 0))[#raw_year])
}
